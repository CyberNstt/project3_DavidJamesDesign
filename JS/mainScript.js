...
const animItems = document.querySelectorAll('.ScrollAnimation');
	...
	function animOnScroll() {
		for (let index = 0; index < animItems.length; index++) {
			...
			if ((pageYOffset > animItemOffset - animItemPoint) && pageYOffset < (animItemOffset + animItemHeight)) {
				animItem.classList.add('AnimOnScrollActive');
			} else {
				if (!animItem.classList.contains('AnimOnScrollNoBack')) {
					animItem.classList.remove('AnimOnScrollActive');
				}
			}
		}
	}
